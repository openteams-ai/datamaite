...
:class:`HmieLoader` is datamaite's first :class:`~datamaite.loaders.Loader`:
it reads the HMIE/Scale on-disk layout and produces the MAITE-MOT-capable
:class:`datamaite.model.BoxTrackDataset` that every converter consumes. The
generic, format-agnostic machinery (the ``Loader`` base class, the registry,
and the :func:`datamaite.load` dispatcher) lives in
:mod:`datamaite.loaders`; this module is what a new format loader is modeled
...
:func:`datamaite._formats.hmie.discovery.discover_hmie_pairs` to pair
...
annotation is bad should run :func:`datamaite.validate` instead.
"""
...
from datamaite._formats.hmie.annotation_checks import _is_unwrapped_annotations, check_annotation_schema
...
class HmieLoader(Loader):
    ...
    Subclasses :class:`datamaite.loaders.Loader` and is registered for
    :attr:`DatasetFormat.HMIE`, so ``datamaite.load(root)`` dispatches here.
    """
...
def _build_boxes(
    ...
    *,
    ...
    (:class:`~datamaite.model.BoxAnnotation`) that positive ids mean a
