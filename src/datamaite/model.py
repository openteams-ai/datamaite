"""Datamaite's neutral in-memory dataset models.
...
bounding-box tracks. A loader (e.g. :func:`datamaite.load_mot`) turns an
...
MAITE-protocol view (:mod:`datamaite.maite`). Because that model is tied to
neither a specific input nor a specific output format, datamaite is an N-to-M
...
universal model for every future datamaite format. Tasks whose labels are not
...
protocol via lazily-imported view machinery in :mod:`datamaite.maite`; no
    ...
    * ``ds.sequence_count`` / ``ds.iter_sequences()`` / ``ds.sequences`` are the
    ...
    def with_mot_options(
        ...
        *,
        ...
        decoder
            Video-decode backend (a ``datamaite.maite._decode.Decoder``);
