...
def _print_batch_table(results: list[tuple[Path, Any]]) -> None:
    ...
    from datamaite._formats.hmie.categories import SKIP_VIDEO_INTEGRITY, _categorize_findings
    from datamaite._types import _dim, _status_indicator
...
def _resolve_output_path(args: argparse.Namespace, root: Path) -> None:
    ...
    if args.output == "auto":
        ...
        args.output = Path(f"datamaite-report-{dataset_name}-{timestamp}.html")
...
def _cmd_stats(args: argparse.Namespace) -> int:
    ...
    from datamaite._stats import dataset_stats, format_stats
...
def _print_result(args: argparse.Namespace, result: Any) -> None:
    ...
    if args.output:
        ...
        if ext == ".html":
            from datamaite._report import render_html_report
...
def _run_batches(
    ...
    """
    from datamaite._types import DatasetFormat
    from datamaite.validation import _build_validate_crash_result, validate
...
def _validate_multi(
    ...
    print(f"    datamaite validate {root}/<batch-name>")
    print()
